...
public class EntityCard extends EntityStacked {
    ...
    public EntityCard(Level world, Vec3 position, float rotation, byte skinID, UUID deckUUID, boolean covered, byte firstCardID) {
        super(Services.REGISTRY.getCardEntityType().get(), world, position);
    ...
    private void takeCard(Player player) {
        ...
        ItemStack card = new ItemStack(Services.REGISTRY.getCard().get());
        if (this.entityData.get(COVERED)) card = new ItemStack(Services.REGISTRY.getCardCovered().get());

        card.set(Services.DATA.getValue().get(), getTopStackID());
        card.set(Services.DATA.getDeckId().get(), getDeckUUID());
        card.set(Services.DATA.getSkinId().get(), this.entityData.get(SKIN_ID));
        card.set(Services.DATA.getCovered().get(), this.entityData.get(COVERED));
