...
public class EntityPokerChip extends EntityStacked {
    ...
    public EntityPokerChip(Level world, Vec3 position, UUID ownerID, String ownerName, byte firstChipID) {
        super(Services.REGISTRY.getPokerChipEntityType().get(), world, position);
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        ...
        if (pSource.getDirectEntity() instanceof Player player) {
            ...
            if (whiteAmount > 0) spawnChip(player, Services.REGISTRY.getPokerChipWhite().get(), whiteAmount);
            if (redAmount > 0) spawnChip(player, Services.REGISTRY.getPokerChipRed().get(), redAmount);
            if (blueAmount > 0) spawnChip(player, Services.REGISTRY.getPokerChipBlue().get(), blueAmount);
            if (greenAmount > 0) spawnChip(player, Services.REGISTRY.getPokerChipGreen().get(), greenAmount);
            if (blackAmount > 0) spawnChip(player, Services.REGISTRY.getPokerChipBlack().get(), blackAmount);
    ...
    private void spawnChip(Player player, Item item, int amount) {
        ...
        if (!level().isClientSide) {
            ...
            chip.set(Services.DATA.getOwnerId().get(), getOwnerUUID());
            chip.set(Services.DATA.getOwnerName().get(), this.entityData.get(OWNER_NAME));
