...
public class ItemPokerChip extends ItemBase {
    ...
    public static Item getPokerChip(byte pokerChipID) {
        ...
        switch (pokerChipID) {
            case 1:
                return Services.REGISTRY.getPokerChipRed().get();
            case 2:
                return Services.REGISTRY.getPokerChipBlue().get();
            case 3:
                return Services.REGISTRY.getPokerChipGreen().get();
            case 4:
                return Services.REGISTRY.getPokerChipBlack().get();
        }

        return Services.REGISTRY.getPokerChipWhite().get();
    }

    @Override
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        UUID ownerID = stack.get(Services.DATA.getOwnerId().get());

        if (ownerID != null) {
            tooltipComponents.add(Component.literal(ChatFormatting.GRAY + "Owner: " + ChatFormatting.GOLD + stack.get(Services.DATA.getOwnerName().get())));
        }

        else tooltipComponents.add(Component.literal(ChatFormatting.GRAY + "Owner: " + ChatFormatting.GOLD + "Not set"));

        tooltipComponents.add(Component.literal(ChatFormatting.GRAY + "Value (1): " + ChatFormatting.GOLD + value));

        if (stack.getCount() > 1) {
            tooltipComponents.add(Component.literal(ChatFormatting.GRAY + "Value (" + stack.getCount() + "): " + ChatFormatting.GOLD + StringHelper.printCommas(value * stack.getCount())));
        }
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (pPlayer.isCrouching()) {
            ...
            UUID ownerID = heldItem.get(Services.DATA.getOwnerId().get());

            if (ownerID == null) {

                heldItem.set(Services.DATA.getOwnerId().get(), pPlayer.getUUID());
                heldItem.set(Services.DATA.getOwnerName().get(), pPlayer.getDisplayName().getString());
        ...
        if (player != null) {
            ...
            if (!player.isCrouching()) {
                ...
                if (ownerID != null) {
                    ...
                    String ownerName = stack.get(Services.DATA.getOwnerName().get());
