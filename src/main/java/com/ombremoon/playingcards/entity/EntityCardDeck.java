    ...
    public InteractionResult interact(Player pPlayer, InteractionHand pHand) {
        if (pHand == InteractionHand.MAIN_HAND) {
            ...
            if (getStackAmount() > 0) {
                ...
                card.set(InitDataComponents.VALUE, cardID);
                card.set(InitDataComponents.DECK_ID, getUUID());
                card.set(InitDataComponents.SKIN_ID, this.entityData.get(SKIN_ID));
                card.set(InitDataComponents.COVERED, true);
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (pSource.getDirectEntity() instanceof Player player) {
            ...
            if (player.isCrouching()) {
                ...
                deck.set(InitDataComponents.SKIN_ID, this.entityData.get(SKIN_ID));
